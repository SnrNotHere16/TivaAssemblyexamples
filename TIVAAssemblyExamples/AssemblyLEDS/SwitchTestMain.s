...
;----------------------------------------------------------------
    ...
    MOV R7, #0x00                 	; R7 = 0, intialize counter
	MOV R8, #DARK 					; R8 = 0, DARK
;Define array
loop
    BL  Board_Input
    CMP R0, #0x01                   ; R0 == 0x01?
...
sw2pressed
    ...
    B   loop
bothpressed
	ADD R7, R7, #0x01				;increment R7 by one
	AND R7, R7, #0x03
	CMP R7, #0x00
	BEQ changepink
	CMP R7, #0x01
	...
	B delayMS
bothpressedfin 
	STR R9, [R4]                    ; [R4] = R9 = LED color
	B   loop
nopressed
    STR R8, [R4]                    ; [R4] = R8 = 0 (no LEDs on)
    B   loop
changegreen 						;change color to LED 
	MOV R9, #GREEN					;R9 = GREEN (0x08)
	B delayMS
changepink 							;change color to pink
	MOV R9, #PINK					;R9 = PINK (0x06)
	B delayMS
changeyellow 						;change color to yellow
	MOV R9, #YELLOW					;R9 = YELLOW (0x0A)
	B delayMS
changewhite 						;change color to white
	MOV R9, #WHITE					;R9 = WHITE (0x0E)
	B delayMS
changeskyblue 						;change color to skyblue 
	MOV R9, #SKYBLUE				;R9 = SKYBLUE (0x0C)
	B delayMS
